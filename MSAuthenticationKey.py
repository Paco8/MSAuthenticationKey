...
class Main(object):
    ...
    def __init__(self, browser_temp_path):
        show_msg('')
        show_msg('MSAuthentication Key para Linux/MacOS/Windows (Versión {})'.format(self.app_version),
        ...
        browser_proc = None
        try:
            input_msg('Pulsa tecla "INTRO" para aceptar el descargo de responsabilidad y empezar,\r\no "CTRL+C" para cancelar', TextFormat.BOLD)
            ...
            show_msg('Se ha producido un error:\r\n' + str(exc), TextFormat.COL_LIGHT_RED)
            import traceback
    ...
    def operations(self):
        show_msg('Estableciendo conexión con el navegador en el puerto {}... por favor espera'.format(DEBUG_PORT))
        self.get_browser_debug_endpoint()
        self.ws_request('Network.enable')
        self.ws_request('Page.enable')
        show_msg('Cargando la página web... por favor espera')
        ...
        show_msg('Por favor inicia sesión en la web. Esperando a que acabes...', TextFormat.COL_LIGHT_BLUE)
    ...
    def wait_login(self):
        ...
        while time.time() - start_time < 300:  # 5 min
            ...
            if'method' in parsed_message:
              if parsed_message['method'] == 'Network.requestWillBeSent':
                #print('m: {}'.format(json.dumps(parsed_message, indent=4)))
                url = parsed_message['params']['request']['url']
                #print('url: {}'.format(url))
                if '/token' in url:
                  #print('m: {}'.format(json.dumps(parsed_message, indent=4)))
                  if parsed_message['params']['request']['method'] == 'POST':
                    request_id = parsed_message['params']['requestId']
                    res['request_headers'] = parsed_message['params']['request']['headers']
              if request_id and parsed_message['method'] == 'Network.loadingFinished' and request_id == parsed_message['params']['requestId']:
                message = json.dumps({'id': req_id, 'method': 'Network.getResponseBody', 'params': {"requestId": request_id}})
                self._ws.send(message)
            if 'result' in parsed_message and parsed_message['id'] == req_id:
              res['response'] = json.loads(parsed_message['result']['body'])
              break
        return res
...
if __name__ == '__main__':
    ...
    if IS_WINDOWS:
        input_msg('Pulsa "INTRO" para acabar', TextFormat.BOLD)
